...
**本月必学：**前三段的核心概念。纠缠重点建立 Bell 态直觉；完整协议、密度矩阵与信道在 M02 学习。
...
| Q4 | [Watrous 统一讲义](https://arxiv.org/abs/2507.11536) | 与 IBM 课程对应的讲义阅读入口 |
...
前沿定位围绕“这个基础概念将用于哪类问题”展开。忙碌周优先完成核心概念，实践与补缺可安排到机动周。
...
| B · 2h | 从 D2L 第 3 章浏览一个现成模型流程；指出输入、预测、损失、更新和评价 | 能指出优化目标与最终指标的区别 |
...
| C · 1h | 总结 underfit/overfit 的判断；核对数据划分、训练预算等条件 | 结合训练与验证表现判断拟合情况 |
...
| F · 1h | 连接分类评估与量子实验观察；记录下周想检验的一个问题 | 确定验证问题、条件与观察指标 |
...
| F · 1h | 月末检查、更新进度、给 M02 留出最弱先修点 | 根据自检结果决定进入 M02 或安排补缺周 |

## 08 · 月末通过标准

请将下列能力标为“能解释 / 需要例子帮助 / 仍不清楚”，并链接自己的记录。

| 能力 | 本月结束时的目标 |
| :--- | :--- |
| ML 任务与评估 | 说明数据划分、loss/metric、泛化与正则化；识别一个泄漏或分布变化风险 |
| 模型流程 | 看懂现成训练例子的关键环节，解释数据与参数的变化 |
